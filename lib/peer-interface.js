  ...
  async getAverageSolutionTime(minerAddress)
  {
   ...
   async monitorMinedPayments()
   {
    ...
    console.log('error',e)
      ...
      setTimeout(function(){self.monitorMinedPayments()},40*1000)
...
*/
   async checkMinedPayments(balance_xfers)
   {
     ...
     var REBROADCAST_WAIT_BLOCKS = 550;
     ...
     for(i in balance_xfers)
     {
      ...
      if(transferData != null && transferData.confirmed == true  )
      {
       ...
       if(transferData != null && transferData.txHash != null && transferData.confirmed != true)
		...
		//DEBUG
		/* console.log('liveTransactionReceiptStatus: ',liveTransactionReceipt.status)
			if(liveTransactionReceipt.status == 1 )
			{
			 console.log('transaction_succeeded')
			}else{
			 console.log('transaction_failed')
			}
		*/

            var transaction_succeeded =  (liveTransactionReceipt.status == 1 )
            ...
            await this.redisInterface.storeRedisHashData('balance_transfer:'+minerAddress.toString() ,paymentId,JSON.stringify(transferData) )
